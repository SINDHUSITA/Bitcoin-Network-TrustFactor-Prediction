...
for i in tqdm(nodes_data.index):
  ...
  if len(sent_rows) > 0:
    for index, row in sent_rows.iterrows():
      utc_time = datetime.strptime(row['created at(UTC)'], "%Y-%m-%d %H:%M:%S")
      epoch_time = (utc_time - datetime(1970, 1, 1)).total_seconds()
      sentiment_val = sentiment_data['cluster'][row['id']]
      credibility_sum += ((row['rating'] * epoch_time / curr_time) * (1 + alpha * sentiment_val))

  recv_rows = network_data.loc[network_data['rated nick'] == nodes_data['nick'][i]]
  goodness_sum = 0

  if len(recv_rows) > 0:
    for index,row in recv_rows.iterrows():
      utc_time = datetime.strptime(row['created at(UTC)'], "%Y-%m-%d %H:%M:%S")
      epoch_time = (utc_time - datetime(1970, 1, 1)).total_seconds()
      sentiment_val = sentiment_data['cluster'][row['id']]
      goodness_sum += ((row['rating'] * epoch_time / curr_time) * (1 + alpha * sentiment_val))
  ...
  if len(recv_rows) > 0:
    avg_good = goodness_sum / len(recv_rows)
  
  tf = avg_cred + avg_good
  nodes_data['tf'][i] = tf

r = np.asarray(nodes_data['tf'])
normalized = (r-np.min(r))/(np.max(r)-np.min(r))
print(normalized)
